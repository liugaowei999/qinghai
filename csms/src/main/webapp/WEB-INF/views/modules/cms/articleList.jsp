...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			  //判断当前的栏目是不是父级栏目
			  var childCount = parseInt($("#childLen").val());
			  if(childCount>0){
				  $("#addArticle").hide();
			  }
			});
...
<body>
	<ul class="nav nav-tabs">
		...
		<li id="addArticle"><a href="<c:url value='${fns:getAdminPath()}/cms/article/form?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">文章添加</a></li>
		</shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="article" action="${ctx}/cms/article/" method="post" class="breadcrumb form-search">
		...
		<input id="childLen" name="childLen" type="hidden" value="${article.childCount}"/>
