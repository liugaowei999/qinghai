...
//		return dao.find(page, dc);
	...
	public void save(Article article) {
		...
		String refreshId="csms/f/view-"+article.getCategory().getId()+"-"+article.getId()+".html";
		HttpRequestUtil.sendGetNotRes(nginxServer+refreshId, null);
//		System.out.println("❀❀❀❀❀❀❀❀❀❀❀❀❀❀❀❀❀❀❀❀❀❀❀CategoryId=="+article.getCategory().getId()+"❀❀❀❀❀id=="+article.getId());
		/////写入solr,创建索引
...
//		dao.updateDelFlag(id, isRe!=null&&isRe?Article.DEL_FLAG_NORMAL:Article.DEL_FLAG_DELETE);
		...
		HttpRequestUtil.sendGetNotRes(nginxServer+refreshId, null);
		///从solr中删除
		articleIndexService.delete(article.getId());
	}
...
//		System.out.println(queryFilter);
	...
	public Page<Article> search(Page<Article> page, String q){
		HighlightPage<ArticleDocument>  hlpage = articleIndexService.hlsearch(q, page.getPageNo(), page.getPageSize());
		List<HighlightEntry<ArticleDocument>>  hllist = hlpage.getHighlighted();
		List<Article> resultList = new ArrayList<Article>();
		for(HighlightEntry<ArticleDocument> each:hllist){
			List<Highlight>  hls = each.getHighlights();
			ArticleDocument doc = each.getEntity();
			if(hls.size()>0){
				Article article = new Article();
				article.setId(doc.getId());
				for(Highlight hl:hls){
					if(hl.getField().getName().equals("title")){
						article.setTitle(hl.getSnipplets().get(0));
					}
					if(hl.getField().getName().equals("content")){
						article.setDescription(hl.getSnipplets().get(0)+"...");
					}
				}
				...
				if(StringUtils.isBlank(article.getDescription())){
					article.setDescription(StringUtils.substring(doc.getContent(), 0, 100)+"...");
				}
				Category category = new Category();
				category.setId(doc.getCategoryId());
				category.setName(doc.getCategory());
				article.setCategory(category);
				article.setCreateDate(doc.getCreateDate());
				article.setUpdateDate(doc.getUpdateDate());
				User createBy = new User();
				createBy.setName(doc.getAuthor());
				article.setCreateBy(createBy);
				/////获取点击数
				Double count = JedisUtils.zget(CmsUtils.ARTICLE_HIT_COUNT, doc.getId());
				if(count!=null){
					article.setHits(count.intValue());
				}
