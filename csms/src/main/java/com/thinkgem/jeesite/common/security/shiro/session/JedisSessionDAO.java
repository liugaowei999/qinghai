...
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO {
	...
	public void update(Session session) throws UnknownSessionException {
		...
		try {
			...
			JedisUtils.mapPut(sessionKeyPrefix, session.getId().toString(), principalId + "|" + session.getTimeout() + "|" + session.getLastAccessTime().getTime());
			// 设置超期时间
			int timeoutSeconds = (int)(session.getTimeout() / 1000);
			JedisUtils.setObject(sessionKeyPrefix + session.getId(), session, timeoutSeconds);
	...
	public void delete(Session session) {
		...
		try {
			...
			JedisUtils.mapRemove(sessionKeyPrefix, session.getId().toString());
			JedisUtils.del(sessionKeyPrefix + session.getId());
	...
	public Collection<Session> getActiveSessions(boolean includeLeave, Object principal, Session filterSession){
		...
		try {
			Map<String, String> map = JedisUtils.getMap(sessionKeyPrefix);
...
//			if (jedis.exists(sessionKeyPrefix + sessionId)){
				session = (Session)JedisUtils.getObject(sessionKeyPrefix + sessionId);
//			}
